...
import { AddBatchMessageParams, APIResponseProps, BatchSMSErrorsParams, GetSingleUserParams, Send_SMS_Params, SendSMSParams, 
    SentSMSParams, 
    SetBatchMessageStatsParams } from "@/components/types";
...
import { send_SMS } from "@/_lib/send_SMS";
...
export class SMS_Service {
    ...
    public async SendSMS(params: Send_SMS_Params): Promise<APIResponseProps> {

        console.log("made it here SendSMS()")
        const default_resp = {
            message: "",
            data: {},
            success: false,
        ...
        try{
            ...
            const response = await send_SMS(api_info.data.twillio_account_sid, api_info.data.twillio_auth_token, 
                api_info.data.twillio_phone_number, to_number, body); 
            
            if(response == "SMS sent"){
                default_resp.success = true;
                default_resp.message = "SMS sent!";
                default_resp.data = {sent_to: to_number};
                
                const add_params: SentSMSParams={
                    user_id,
                    to_number,
                    body,
                    message_type: params.message_type,
                    form_number: api_info.data.twillio_phone_number,
                }

                if(user_id && user_id > 0){
                    this.sms_repo.AddSentSMS(add_params);
                }
            }else{
                default_resp.message = "Error sending sms:: "+response;
