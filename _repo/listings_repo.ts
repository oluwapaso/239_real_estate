...
export class MysqlListingsRepo implements ListingsRepo {
    ...
    public async GetNextReplicateLink(): Promise<string>{
        ...
        try{
            const [row] = await pool.query<RowDataPacket[]>(`SELECT next_link FROM fetch_listings WHERE next_link!='' `);
    ...
    public async UpdateNextLink(next_link: string): Promise<boolean> {
        ...
        try{
            ...
            const [up_result] = await pool.query<ResultSetHeader>(`UPDATE fetch_listings SET next_link=? `, [next_link]);
            return true;
    ...
    public async AddNewListing(fields: string, values: any[], placeholders: string, update_cond: string): Promise<boolean> {
        ...
        try{
            ...
            const [add_result] = await pool.query<ResultSetHeader>(`INSERT INTO listings(${fields},DateAdded) VALUES(${placeholders},?) 
    ...
    public async LoadListings(req: NextApiRequest, search_filter: string, order_by: string): Promise<any> { //any[]
        ...
        if(search_by == "List"){
            ...
            [rows] = await pool.query<RowDataPacket[]>(`SELECT ${fields}, (SELECT COUNT(*) AS total_records FROM properties WHERE 
            ...
            [rows] = await pool.query<RowDataPacket[]>(`SELECT ${fields} FROM properties WHERE Status='Active' ${search_filter} 
            ...
            [rows] = await pool.query<RowDataPacket[]>(`SELECT ${fieldsRslt}, (SELECT COUNT(*) AS total_records FROM properties AS L 
            ...
            console.log("Hello SoldByUs:", start_from, comp_info.data.mls_office_key);
            [rows] = await pool.query<RowDataPacket[]>(`SELECT ${fields}, (SELECT COUNT(*) AS total_records FROM properties WHERE 
            ...
            if(params.mobile_view == "Map"){
                [rows] = await pool.query<RowDataPacket[]>(`SELECT ${fields} FROM properties WHERE City!="0" ${search_filter} AND ${map_filter} 
            ...
            if((params.mobile_view == "List" && params.screen_width <= 960) || (params.mobile_view == "Map" && params.screen_width > 960)){
                [list_rows] = await pool.query<RowDataPacket[]>(`SELECT ${fields}, (SELECT COUNT(*) AS total_records FROM properties WHERE City!="0" 
    ...
    public async LiveSearch(keyword: string, search_by: string): Promise<any[]>{
        ...
        if(search_by == "City"){
            ...
            [rows] = await pool.query<RowDataPacket[]>(`SELECT DISTINCT CountyOrParish AS location, 'CountyOrParish' as TABLE_NAME FROM properties 
    ...
    public async GetPropsWithoutImage(limit: number): Promise<any[]>{
        ...
        const [rows] = await pool.query<RowDataPacket[]>(`SELECT property_id, Media, FullAddress, City FROM properties WHERE MediaLoaded='No' 
        ...
        if(rows.length){
            ...
            if(property_ids.length){
                ...
                console.log("implodedString:", implodedString)
                const [up_result] = await pool.query<ResultSetHeader>(`UPDATE listings SET MediaLoaded='Loading' WHERE property_id IN('${implodedString}')`);
    ...
    public async AddUploadedImages(property_id:number, medias: any[]): Promise<boolean> {
        ...
        try{
            ...
            const [up_result] = await pool.query<ResultSetHeader>(`UPDATE listings SET MediaLoaded=?, Images=?, LastImageSync=? 
    ...
    public async ResetUploadImageStatus(property_id:number): Promise<boolean> {
        ...
        try{
            ...
            const synched = moment().format('YYYY-MM-DD HH:mm:ss');
            const [up_result] = await pool.query<ResultSetHeader>(`UPDATE listings SET MediaLoaded=?, Images=NULL, LastImageSync=? 
    ...
    public async LoadSingleProperty(prop_key: string): Promise<any>{
        ...
        try{
            const [rows] = await pool.query<RowDataPacket[]>(`SELECT * FROM properties WHERE MLSNumber=? `, [prop_key]);
            if(rows && rows.length){
    ...
    public async LoadSimilarListings(req: NextApiRequest): Promise<any[]> {
        ...
        [rows] = await pool.query<RowDataPacket[]>(`SELECT ${fields} FROM properties WHERE Status='Active' 
    ...
    public async UpdateFavorites(req: NextApiRequest): Promise<any[]> {
        ...
        if(type == "Add") {
            ...
            [up_result] = await pool.query<ResultSetHeader>(`INSERT INTO favorites(matrix_unique_id, user_id, date_added) VALUES(?, ?, ?)`, [listing_id, user_id, now]);
        }else if(type == "Remove") {
            [up_result] = await pool.query<ResultSetHeader>(`DELETE FROM favorites WHERE matrix_unique_id=? AND user_id=?`, [listing_id, user_id]);
        }
        ...
        const [fav_row] = await pool.query<RowDataPacket[]>(`SELECT matrix_unique_id FROM favorites WHERE user_id=?`, [user_id]);
        const favs: any[] = [];
        if(fav_row.length){
            fav_row.forEach(fav=> {
    ...
    public async SaveSearches(req: NextApiRequest): Promise<APIResponseProps> {
        ...
        const [srch_row] = await pool.query<RowDataPacket[]>(`SELECT(SELECT COUNT(search_id) FROM saved_searches WHERE user_id=? AND search_title=?) AS TitleExist,
        ...
        if( srch_row.length > 0){
            ...
            let addResult = "";
            try{
                const [add_result] = await pool.query<ResultSetHeader>(`INSERT INTO saved_searches(user_id, search_title, query_link, query_type, 
    ...
    public async LoadSavedSearches(req: NextApiRequest): Promise<any> {
        ...
        const [rows] = await pool.query<RowDataPacket[]>(`SELECT ${field}, (SELECT COUNT(*) AS total_records FROM saved_searches WHERE 
    ...
    public async DeleteSavedSearches(req: NextApiRequest): Promise<boolean> {
        ...
        try{
            ...
            const [del_srch] = await pool.query<ResultSetHeader>(`DELETE FROM saved_searches WHERE search_id=? AND user_id=? `, [search_id, user_id]);
    ...
    public async GetAlerts(): Promise<any> {
        ...
        const [rows] = await pool.query<RowDataPacket[]>(` SELECT * FROM saved_searches WHERE 
    ...
    public async UpdateAlertedSearches(searh_ids: string): Promise<boolean> {
        ...
        try{
            ...
            const [del_srch] = await pool.query<ResultSetHeader>(`UPDATE saved_searches SET last_alert=?, started=? WHERE search_id IN(${searh_ids}) `, [now, "Yes"]);
            return del_srch.affectedRows >= 0;
    ...
    public async UpdateFeaturedListingsSettings(req: NextApiRequest): Promise<boolean> {
        ...
        try{
            ...
            const [update] = await pool.query<ResultSetHeader>(`UPDATE company_info SET featured_listings=?  WHERE company_id=?`, [settings, "1"]);
    ...
    public async LogSearch(req: NextApiRequest): Promise<boolean> {
        ...
        const [srch_row] = await pool.query<RowDataPacket[]>(`SELECT (SELECT COUNT(search_id) FROM saved_searches WHERE user_id=? AND query_link=?
