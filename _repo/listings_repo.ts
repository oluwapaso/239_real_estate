...
export class MysqlListingsRepo implements ListingsRepo {
    ...
    public async AddNewListing(defaultClass: string, fields: string, values: any[], placeholders: string, update_cond: string, 
        full_info: string): Promise<boolean> {
        ...
        try{
            ...
            const [add_result] = await connection.query<ResultSetHeader>(`INSERT INTO properties(PropertyClass,${fields},DateAdded, full_info) 
            VALUES(?,${placeholders},?, ?) ON DUPLICATE KEY UPDATE ${update_cond}, full_info=?`, [defaultClass, ...values, dateAdded, 
            full_info, full_info]);
        ...
        try{
            ...
            if(rows.length){
                ...
                if(property_ids.length){
                    ...
                    //Set the selected ids to "Loading"
                    const implodedString = property_ids.join("', '");
                    const [up_result] = await connection.query<ResultSetHeader>(`UPDATE properties SET AllPixDownloaded='Loading' 
                    WHERE property_id IN('${implodedString}')`);
    ...
    public async UpdatePropImages(matrix_unique_id: string, images: string, DefaultPic: string): Promise<boolean>{
        ...
        try{
            ...
            const [up_result] = await connection.query<ResultSetHeader>(`UPDATE properties SET AllPixDownloaded=?, DefaultPic=?, 
            Images=?, last_image_query=? WHERE matrix_unique_id=?`, ["Yes", DefaultPic, images, synched, matrix_unique_id]);
    ...
    public async AddUploadedImages(property_id:number, medias: any[]): Promise<boolean> {
        ...
        try{
            ...
            const synched = moment().format('YYYY-MM-DD HH:mm:ss');
            const [up_result] = await connection.query<ResultSetHeader>(`UPDATE properties SET AllPixDownloaded=?, Images=?, last_image_query=? 
    ...
    public async ResetUploadImageStatus(property_id:number): Promise<boolean> {
        ...
        try{
            ...
            connection = await pool.getConnection();
            const synched = moment().format('YYYY-MM-DD HH:mm:ss');
            const [up_result] = await connection.query<ResultSetHeader>(`UPDATE properties SET AllPixDownloaded=?, Images=NULL, last_image_query=? 
    ...
    public async GetAlerts(): Promise<any> {
        ...
        try{
            ...
            (email_frequency = 'Monthly' AND last_alert < NOW() - INTERVAL 1 MONTH) LIMIT 3`); //10
    ...
    public async GetPendingSyncs(): Promise<any[]>{
    ...
    public async GetSyncOffset(defaultClass: any): Promise<number>{

        let connection: PoolConnection | null = null;
        try{

            connection = await pool.getConnection();
            const [rows] = await connection.query<RowDataPacket[]>(`SELECT COUNT(property_id) AS \`offset\` FROM properties 
            WHERE PropertyClass='${defaultClass}'`); 
    ...
    public async GetRefreshSkips(skip_col_date: string, skip_col_count: string): Promise<any[]>{

        let connection: PoolConnection | null = null;
        try{

            connection = await pool.getConnection();
            const [rows] = await connection.query<RowDataPacket[]>(`SELECT ${skip_col_date}, ${skip_col_count} FROM refresh_skips 
            WHERE skip_id='1'`); 
    ...
    public async GetNewestProp(defaultClass: string): Promise<string>{
    ...
    public async UpdateRefreshSkip(query: string): Promise<boolean>{
        
        let connection: PoolConnection | null = null;
        try{

            connection = await pool.getConnection();
            const [del_srch] = await connection.query<ResultSetHeader>(query);
            return del_srch.affectedRows >= 0;
