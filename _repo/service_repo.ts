...
export class MYSQLServiceRepo implements ServiceRepo {
    ...
    public async CheckServiceSlug(params: CheckSlugParams): Promise<number>{
        ...
        try{
            ...
            if(check_by == "Slug"){
                [rows] = await connection.query<RowDataPacket[]>(`SELECT COUNT(*) AS slug_exist FROM service_drafts WHERE slug=?`, [slug]);
            }else if(check_by == "Slug Alt"){
                const draft_id = params?.draft_id;
                [rows] = await connection.query<RowDataPacket[]>(`SELECT COUNT(*) AS slug_exist FROM service_drafts WHERE slug=? AND draft_id!=?`, [slug, draft_id]);
            }
        ...
        try{
            ...
            if(submit_type == "Draft"){
                ...
                if(draft_id < 1){
                    ...
                    const [result] = await connection.query<ResultSetHeader>(`UPDATE service_drafts SET friendly_name=?, slug=?, excerpt=?, 
                ...
                if(draft_id < 1){
                    ...
                    const [del_result] = await connection.query<ResultSetHeader>(`DELETE FROM services WHERE service_draft_id=? `, [draft_id]);
    ...
    public async LoadServiceInfo(slug: string, search_by: string): Promise<ServiceDetails | null> {
        ...
        try{
            ...
            if(search_by == "Slug"){
                [rows] = await connection.query<RowDataPacket[]>(`SELECT * FROM services WHERE slug=? `, [slug]);
            }else if(search_by == "Community Id"){
                [rows] = await connection.query<RowDataPacket[]>(`SELECT * FROM services WHERE service_id=? `, [slug]);
            }
    ...
    public async LoadDraftInfo(draft_id: number): Promise<ServiceDetails | null> {
        ...
        try{
            ...
            connection = await pool.getConnection();
            const [rows] = await connection.query<RowDataPacket[]>(`SELECT * FROM service_drafts WHERE draft_id=? `, [draft_id]);
            if(rows.length){
                const formattedRows = rows.map((row) => {
    ...
    public async LoadServices(params: LoadServicesParams): Promise<ServiceLists[] | null> {
        ...
        try{
            ...
            if(paginated){
                ...
                if(post_type == "Draft"){
                [rows] = await connection.query<RowDataPacket[]>(`SELECT *, (SELECT COUNT(*) AS total_records FROM service_drafts) AS total_records FROM service_drafts ORDER BY friendly_name ASC LIMIT ${start_from}, ${limit}`);
                } else if(post_type == "Published"){
                    [rows] = await connection.query<RowDataPacket[]>(`SELECT *, (SELECT COUNT(*) AS total_records FROM services) AS total_records FROM services ORDER BY friendly_name ASC LIMIT ${start_from}, ${limit}`);
                }
                
            }else{

                if(post_type == "Draft"){
                    [rows] = await connection.query<RowDataPacket[]>(`SELECT * FROM service_drafts ORDER BY friendly_name ASC `);
                } else if(post_type == "Published"){ 
                    [rows] = await connection.query<RowDataPacket[]>(`SELECT * FROM services ORDER BY friendly_name ASC `);
                } else if(post_type == "Featured Services"){ 
                    [rows] = await connection.query<RowDataPacket[]>(`SELECT * FROM services WHERE JSON_EXTRACT(show_on_menus, '$.home_page')=? ORDER BY friendly_name ASC `, ["Yes"]);
                }
