...
export class MYSQLCommunityRepo implements CommunityRepo {
    ...
    public async CheckCommunitySlug(params: CheckSlugParams): Promise<number>{
        ...
        if(check_by == "Slug"){
            [rows] = await pool.query<RowDataPacket[]>(`SELECT COUNT(*) AS slug_exist FROM community_drafts WHERE slug=?`, [slug]);
        }else if(check_by == "Slug Alt"){
            const draft_id = params?.draft_id;
            [rows] = await pool.query<RowDataPacket[]>(`SELECT COUNT(*) AS slug_exist FROM community_drafts WHERE slug=? AND draft_id!=?`, [slug, draft_id]);
        }
        ...
        try{
            ...
            if(submit_type == "Draft"){
                ...
                if(draft_id < 1){
                    ...
                    const [result] = await pool.query<ResultSetHeader>(`UPDATE community_drafts SET city_slug=?, city_id=?, mls_name=?, 
                ...
                if(draft_id < 1){
                    ...
                    const [del_result] = await pool.query<ResultSetHeader>(`DELETE FROM communities WHERE community_draft_id=? `, [draft_id]);
    ...
    public async LoadCommunityInfo(slug: string, search_by: string): Promise<CommunityDetails | null> {
        ...
        try{
            ...
            if(search_by == "Slug"){
                [rows] = await pool.query<RowDataPacket[]>(`SELECT * FROM communities WHERE slug=? `, [slug]);
            }else if(search_by == "Community Id"){
                [rows] = await pool.query<RowDataPacket[]>(`SELECT * FROM communities WHERE community_id=? `, [slug]);
            }
    ...
    public async LoadDraftInfo(draft_id: number): Promise<CommunityDraftsInfo | null> {
        ...
        try{
            const [rows] = await pool.query<RowDataPacket[]>(`SELECT * FROM community_drafts WHERE draft_id=? `, [draft_id]);
            if(rows.length){
                const formattedRows = rows.map((row) => {
    ...
    public async LoadCommunities(params: LoadCommunitiesParams): Promise<CommunityInfo[] | null> {
        ...
        if(paginated){
            ...
            if(post_type == "Draft"){
               [rows] = await pool.query<RowDataPacket[]>(`SELECT *, (SELECT COUNT(*) AS total_records FROM community_drafts) AS total_records FROM community_drafts ORDER BY friendly_name ASC LIMIT ${start_from}, ${limit}`);
            } else if(post_type == "Published"){
                [rows] = await pool.query<RowDataPacket[]>(`SELECT *, (SELECT COUNT(*) AS total_records FROM communities) AS total_records FROM communities ORDER BY friendly_name ASC LIMIT ${start_from}, ${limit}`);
            } else if(post_type == "City's Community"){
                const city_slug = params.city_slug;
                [rows] = await pool.query<RowDataPacket[]>(`SELECT *, (SELECT COUNT(*) AS total_records FROM communities WHERE city_slug=?) 
            ...
            if(post_type == "Draft"){
                [rows] = await pool.query<RowDataPacket[]>(`SELECT * FROM community_drafts ORDER BY friendly_name ASC `);
            } else if(post_type == "Published"){ 
                [rows] = await pool.query<RowDataPacket[]>(`SELECT * FROM communities ORDER BY friendly_name ASC `);
            } else if(post_type == "Featured Communities"){ 
                [rows] = await pool.query<RowDataPacket[]>(`SELECT * FROM communities WHERE JSON_EXTRACT(show_on_menus, '$.home_page')=? ORDER BY friendly_name ASC `, ["Yes"]);
            }
    ...
    public async DeleteCommunity(draft_id: number): Promise<boolean> {
        ...
        try{
            ...
            const [del_draft] = await pool.query<ResultSetHeader>(`DELETE FROM community_drafts WHERE draft_id=? `, [draft_id]);
            const [del_post] = await pool.query<ResultSetHeader>(`DELETE FROM communities WHERE community_draft_id=? `, [draft_id]);
    ...
    public async LoadCommunityData(community: string): Promise<APIResponseProps> {
        ...
        try{
            ...
            const [data] = await pool.query<RowDataPacket[]>(`SELECT
