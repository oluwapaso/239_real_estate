...
export class MYSQLUserRepo implements UserRepo {
    ...
    public async UserLogin(params: UserLoginParams): Promise<any>{
        ...
        try{
            ...
            const [user_row] = await pool.query<RowDataPacket[]>(`SELECT * FROM users WHERE email=?`, [email]);
            if(user_row.length){
                ...
                const isValidPwrd = await bcrypt.compare(password, hashed_password).then(async (result) => {
                    ...
                    if (result) {
                        ...
                        const [sess_result] = await pool.query<ResultSetHeader>(`INSERT INTO user_session(user_id, token, expire_on) VALUES (?, ?, ?) `, 
                        [userInfo.user_id, token, expire_on]);
                        
                        if(sess_result.affectedRows > 0){

                            const [fav_row] = await pool.query<RowDataPacket[]>(`SELECT matrix_unique_id FROM favorites WHERE user_id=?`, [userInfo.user_id]);
                            const favs: any[] = [];
                            if(fav_row.length){
                                fav_row.forEach(fav=> {
    ...
    public async UserAuthLogin(params: UserAuthParams): Promise<any>{
        ...
        try{
            ...
            if(user_row.length){
                ...
                const [fav_row] = await pool.query<RowDataPacket[]>(`SELECT matrix_unique_id FROM favorites WHERE user_id=?`, [user_info.user_id]);
                const favs: any[] = [];
                if(fav_row.length){
                    fav_row.forEach(fav=> {
                        favs.push(fav.matrix_unique_id);
                ...
                const [sess_result] = await pool.query<ResultSetHeader>(`INSERT INTO users(email, firstname, lastname, password) VALUES (?, ?, ?, ?) `, 
    ...
    public async GetSingleUser({params}:{params: GetSingleUserParams}): Promise<User | string | null>{
        ...
        if(search_type == "Email"){
            ...
            try{
                const [row] = await pool.query<RowDataPacket[]>(`SELECT ${field} FROM users WHERE email=? `, [email]);
            ...
            try{
                const [row] = await pool.query<RowDataPacket[]>(`SELECT ${field} FROM users WHERE email=? AND user_id!=?`, [email, user_id]);
                return row.length ? row[0] as User : null
            }catch(e: any){
            ...
            try{
                const [row] = await pool.query<RowDataPacket[]>(`SELECT ${field} FROM users WHERE user_id=? `, [user_id]);
                return row.length ? row[0] as User : null
            }catch(e: any){
                return e.sqlMessage
            }
    ...
    public async AddNewToken({params}:{params: AddTokenParams}): Promise<boolean> {
        ...
        try{
            ...
            await pool.query(`UPDATE users SET status='Reset Password' WHERE email=? `, [email]);
    ...
    public async UpdateAccountPassword({params}:{params: UpUserPasswordParams}): Promise<boolean> {
        ...
        try{
            ...
            const [up_result] = await pool.query<ResultSetHeader>(`UPDATE users SET password=?, status=? WHERE user_id=? `, [password, 'Active', user_id]);
    ...
    public async UpdateAccountDetails(req: NextApiRequest): Promise<boolean> {
        ...
        try{
            ...
            const [up_result] = await pool.query<ResultSetHeader>(`UPDATE users SET email=?, secondary_email=?, firstname=?, lastname=?, 
    ...
    public async UpdateLeadDetails(req: NextApiRequest): Promise<boolean> {
        ...
        try{
            ...
            const [up_result] = await pool.query<ResultSetHeader>(`UPDATE users SET email=?, secondary_email=?, firstname=?, lastname=?, 
            phone_1=?, phone_2=?, work_phone=?, fax=?, street_address=?, city=?, state=?, zip=?, price_range=?, spouse_name=?, 
            ...
            req_body.spouse_name, birthday, req_body.facebook, req_body.linkedin, req_body.twitter, req_body.tictoc, req_body.whatsapp, 
            req_body.background, req_body.lead_stage, req_body.user_id]);
            return up_result.affectedRows > 0;
    ...
    public async DeleteAccount(req: NextApiRequest): Promise<boolean> {
        ...
        try{
            ...
            const [fav_result] = await pool.query<ResultSetHeader>(`DELETE FROM favorites WHERE user_id=? `, [user_id]);
            const [srch_result] = await pool.query<ResultSetHeader>(`DELETE FROM saved_searches WHERE user_id=? `, [user_id]);
            const [sess_result] = await pool.query<ResultSetHeader>(`DELETE FROM user_session WHERE user_id=? `, [user_id]);
            const [del_result] = await pool.query<ResultSetHeader>(`DELETE FROM users WHERE user_id=? `, [user_id]);
    ...
    public async UpdateLastSeen(req: NextApiRequest): Promise<boolean> {
        ...
        try{
            const [result] = await pool.query<ResultSetHeader>(`UPDATE users SET last_seen=? WHERE user_id=? `, [last_seen, user_id]);
            return result.affectedRows > 0
        }catch(e: any){
            console.log(e.sqlMessage)
            return false
    ...
    public async LoadUsers(params: LoadUsersParams): Promise<User[] | null> {
        ...
        if(paginated){
            ...
            if(search_type == "User Lists"){
                [rows] = await pool.query<RowDataPacket[]>(`SELECT *, (SELECT COUNT(*) AS total_records FROM users WHERE user_id IS NOT NULL 
    ...
    public async LoadUsersActivities(params: LoadUsersActivities): Promise<any | null> {
        ...
        if(type == "All"){
            ...
            [rows] = await pool.query<RowDataPacket[]>(`SELECT user_id, 'notes' AS table_name, note_id AS activity_id, notes AS description, 
            ...
            [rows] = await pool.query<RowDataPacket[]>(`SELECT user_id, '${table_name}' AS table_name, message_id AS activity_id, message_body AS 
            ...
            [rows] = await pool.query<RowDataPacket[]>(`SELECT user_id, 'property_requests' AS table_name, request_id AS activity_id, 
            ...
            [rows] = await pool.query<RowDataPacket[]>(`SELECT ${fieldsRslt}, F.date_added FROM properties AS L JOIN favorites AS F 
            ...
            [rows] = await pool.query<RowDataPacket[]>(`SELECT ${fields} FROM saved_searches WHERE user_id='${user_id}' ORDER BY date_saved 
    ...
    public async AddSellingRequest(req: NextApiRequest): Promise<boolean> {
        ...
        try{
            const [result] = await pool.query<ResultSetHeader>(`INSERT INTO property_requests(user_id, request_type, request_info, date_added) 
    ...
    public async LogPropertyView(req: NextApiRequest): Promise<boolean> {
        ...
        try{
            ...
            let toatal_view = 0;
            const [check_view] = await pool.query<RowDataPacket[]>(`SELECT COUNT(*) AS total_views FROM property_views WHERE user_id='${user_id}' AND listing_key='${listing_key}' AND date_added > DATE_SUB(NOW(), INTERVAL 30 MINUTE)`);
