...
export class MYSQLCityRepo implements CityRepo { 
    ...
    public async LoadCityInfo(keyword: string, search_by: string): Promise<CityDetails | null> {
        ...
        try{
            ...
            if(search_by == "City Id"){
                [rows] = await pool.query<RowDataPacket[]>(`SELECT * FROM cities WHERE city_id=? `, [keyword]);
            }else if(search_by == "Slug"){
                [rows] = await pool.query<RowDataPacket[]>(`SELECT * FROM cities WHERE slug=? `, [keyword]);
            }
    ...
    public async LoadCities(params: LoadCitiesParams): Promise<CityInfo[] | null> {
        ...
        if(paginated){
            ...
            [rows] = await pool.query<RowDataPacket[]>(`SELECT *, (SELECT COUNT(*) AS total_records FROM cities) AS total_records 
            FROM cities ORDER BY friendly_name ASC LIMIT ${start_from}, ${limit}`);
            
        }else{

            if(post_type == "Published"){ 
                [rows] = await pool.query<RowDataPacket[]>(`SELECT * FROM cities ORDER BY friendly_name ASC `);
            } else if(post_type == "Featured Communities"){ 
                [rows] = await pool.query<RowDataPacket[]>(`SELECT * FROM cities WHERE JSON_EXTRACT(show_on_menus, '$.home_page')=? ORDER BY friendly_name ASC `, ["Yes"]);
            }
