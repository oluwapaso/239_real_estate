...
const ServicesMenuLists = ({ handleCheckedMenu }: { handleCheckedMenu: (event: React.ChangeEvent<HTMLInputElement>) => void }) => {
    return (
        <div className='w-full mt-2 flex flex-col'>
            ...
            <div className='w-full mt-2 mb-2 flex_ items-center select-none hidden'>
                <input type='checkbox' className='styled-checkbox menu_cb' name={`menu_lists`} id={`menu_lists`}
                    onChange={(e) => handleCheckedMenu(e)} />
                <label htmlFor='lists' className='flex w-full'><span>Service Menu List</span></label>
            </div>
        </div>
    )
}

export default ServicesMenuLists