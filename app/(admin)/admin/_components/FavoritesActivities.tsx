...
const FavoritesActivities = ({ user_id, setActivityCounts }: { user_id: number, setActivityCounts: React.Dispatch<React.SetStateAction<InitialActivities>> }) => {
    ...
    useEffect(() => {
        ...
        const fetchActivies = async () => {
            ...
            try {
                ...
                const actsPromise: Promise<any> = helpers.LoadUsersActivities(user_id as unknown as number, "Favorites", skip, limit);
                    ...
                    <ul className="list-none">
                        {
                            !isLoading && activities && activities.length && (
                                activities.map((activity, index) => {
                                    return <FavoritesActivity key={index} activity={activity} index={index} activities_len={activities.length} />
                                })
                            )
                        }
                    </ul>
...
export default FavoritesActivities