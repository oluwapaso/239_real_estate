...
const MoreBlogActions = ({ post_id, draft_id, slug, published, handleDelete }:
    { post_id: number, draft_id: number, slug: string, published: string, handleDelete: (draft_id: number) => Promise<void> }) => {
                    ...
                    <ul className='w-[140px] bg-white shadow-xl border border-gray-200 rounded absolute right-0 z-20 *:px-4 *:py-4 *:cursor-pointer
                     divide-y divide-gray-200'>
                        <li className='hover:bg-gray-50' onClick={() => {
                            ...
                            GoTo(`/admin/edit-blog-post?draft_id=${draft_id}`)
                        }}>
                        ...
                        </li>
                        {
                            published == "Yes" && (
                                <>
                                    <li className='hover:bg-gray-50' onClick={() => openInNewTab(`/read-post/${slug}`)}>
                                        <div className='w-full flex items-center'>
                                    ...
                                    <li className='hover:bg-gray-50' onClick={() => {
                                        dispatch(showPageLoader());
                                        GoTo(`/admin/blog-comments?post_id=${draft_id}&page=1`);
                                    }}>
                                        <div className='w-full flex items-center'>
                                            <LiaCommentsSolid size={16} /> <span className='font-normal ml-1'>Comments</span>
                                        </div>
                                    </li>
                                </>
                            )
...
export default MoreBlogActions