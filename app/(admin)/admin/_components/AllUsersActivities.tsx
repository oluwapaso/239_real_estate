...
import NoteActivity from './NotesActivity';
import SMSActivity from './SMSActivity';
import PropertyRequestActivity from './PropertyReqActivity';

const helpers = new Helpers();
const AllUsersActivities = ({ user_id, setActivityCounts }:
    { user_id: number, setActivityCounts: React.Dispatch<React.SetStateAction<InitialActivities>> }) => {
    ...
    useEffect(() => {
        ...
        const fetchActivies = async () => {
            ...
            try {
                ...
                const actsPromise: Promise<any> = helpers.LoadUsersActivities(user_id as unknown as number, "All", skip, limit);
                    ...
                    <ul className="list-none">
                        {
                            !isLoading && activities && activities.length && (
                                activities.map((activity, index) => {
                                    ...
                                    } else if (activity.table_name == "notes") {
                                        activity_comp = <NoteActivity key={index} activity={activity} index={index} activities_len={activities.length} />
                                    } else if (activity.table_name == "property_requests") {
                                        activity_comp = <PropertyRequestActivity key={index} activity={activity} index={index} activities_len={activities.length} />
                                    }
