...
const PropertyActivities = ({ user_id, setActivityCounts, activity_type }:
    { user_id: number, setActivityCounts: React.Dispatch<React.SetStateAction<InitialActivities>>, activity_type: string }) => {
    ...
    useEffect(() => {
        ...
        const fetchActivies = async () => {
            ...
            try {
                ...
                const actsPromise: Promise<any> = helpers.LoadUsersActivities(user_id as unknown as number, activity_type, skip, limit);
                    ...
                    <ul className="list-none">
                        {
                            !isLoading && activities && activities.length && (
                                activities.map((activity, index) => {
                                    return <PropertyRequestActivity key={index} activity={activity} index={index} activities_len={activities.length} />
                                })
                            )
                        }
                    </ul>
