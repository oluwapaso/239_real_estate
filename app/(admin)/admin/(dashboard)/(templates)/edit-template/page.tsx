...
import { APIResponseProps, TemplateDetails } from '@/components/types';
...
const EditTemplate = () => {
    ...
    useEffect(() => {
        ...
        const fetchDraftInfo = async () => {
            try {
                ...
                const postPromise: Promise<TemplateDetails> = helpers.LoadSingleTemplate(template_id as string);
                const data_info = await postPromise;

                if (!data_info || data_info == null) {
                    setPostFound(false);
                }

                setTempType(data_info.template_type);
                setMailBody(data_info.email_body);
                setSMSBody(data_info.sms_body);

                setInitialValues({
                    template_type: data_info.template_type,
    ...
    return (
        <div className='w-full'>
            <PageTitle text="Update Template" show_back={true} />
            {post_found ?
