...
import UserAutomations from '@/app/(admin)/admin/_components/UserAutomations';
import NewUserAutomationModal from '@/app/(admin)/admin/_components/NewUserAutomationModal';
...
const UserDetails = () => {
    ...
    const [refresh_automations, setRefreshAutomations] = useState(true);
    const [add_automation_resp, setAddAutomationResponse] = useState<any>({});
    ...
    const handleAutomationModal = () => {
        setShowModal(true);
        setModalChild(<NewUserAutomationModal closeModal={closeModal} user_id={user_info.user_id} setAddAutomationResponse={setAddAutomationResponse} />);
        setModalTitle(<>Add New Automation</>)
    }
    ...
    useEffect(() => {
        ...
        const data = add_automation_resp;
        dispatch(hidePageLoader());
        //toast.dismiss();
        if (data.success) {
        ...
        <div className='w-full bg-gray-50 relative'>
            ...
            <div className='w-full grid grid-cols-8 gap-3 mt-4 *:rounded-md *:bg-white *:drop-shadow'>
                ...
                <div className='col-span-4 !bg-transparent'>
                    ...
                    *:py-2 *:px-4 *:cursor-pointer *:text-gray-400'>
                    ...
                    {active_tab == "Send Email" && <ComposeMail user_id={user_id as unknown as number} user_email={user_info.email}
                        setRefreshActivities={setRefreshActivities} />}
                    {active_tab == "Text" && <ComposeSMS phone_number={user_info.phone_1} user_id={user_id as unknown as number}
                        setRefreshActivities={setRefreshActivities} />}
                ...
                <div className='col-span-2 !bg-transparent relative'>
                    <div className='w-full sticky top-0 max-h-[100dvh] overflow-y-auto'>
                        ...
                        <UserAutomations handleAutomationModal={handleAutomationModal} user_id={user_id as unknown as number}
                            refresh_automations={refresh_automations} setRefreshAutomations={setRefreshAutomations} />
                    </div>
                </div>
