...
import { CheckedItems, Task, User } from '@/components/types';
...
const AllTasks = () => {
    ...
    const params_type = searchParams?.get("type") as string || "Upcoming Tasks";
    const initialPage = parseInt(searchParams?.get("page") as string) || 1;

    const [tasks, setTasks] = useState<Task[]>([]);
    const [tasks_fetched, setTasksFetched] = useState(false);
    ...
    useEffect(() => {
        ...
        const fetchTasks = async () => {
            ...
            try {
                ...
                const payload = {
                    paginated: true,
                    search_type: "Tasks Lists",
                    task_type: type,
                    page: initialPage,
                    limit: page_size
                }

                const tasksPromise: Promise<Task[]> = helpers.LoadTasks(payload);
                const tasksResp = await tasksPromise;

                if (tasksResp && tasksResp.length) {
                    const total_records = tasksResp[0].total_records;
                    if (total_records) {
    ...
    const DoSearch = (new_type?: string) => {
        ...
        let link = `/admin/tasks?type=${taskType}&page=1`;
    ...
    const handleCheckAllChange = (uncheck_all?: string) => {
        ...
        const newCheckedItems: CheckedItems = tasks.reduce((acc, task) => {
            acc[task.task_id] = newCheckAll;
            return acc;
        }, {} as CheckedItems);
    ...
    const handleCheckboxChange = (id: number) => {
        setCheckedItems((prev) => {
            ...
            setIsCheckAll(Object.keys(newCheckedItems).length === tasks.length && Object.values(newCheckedItems).every(Boolean));
    ...
    const MarkMultipleAsDone = async () => {
        ...
        toast.dismiss();
        if (num_items < 1) {
            toast.error("Select at least one task to update", {
                position: "top-center",
                theme: "colored"
            });
            return false;
        ...
        const result = await Swal.fire({
            title: "Are you sure you want to mark selected tasks as done?",
        ...
        if (result.isConfirmed) {
            ...
            try {
                dispatch(showPageLoader());
                await helpers.MarkMultipleTasksAsDone(trueKeys);
                setUncheckAll(true);
            } catch (e: any) {
                console.log(e.message);
            }
            ...
            <div className='!w-full !max-w-[100%] h-[auto] relative box-border pb-10'>
                <div className='w-full mt-8 flex justify-between'>
                    <div className="relative">
                        ...
                        {
                            params_type != "Completed Tasks" ? <div className='flex items-center px-5 bg-zinc-900 text-white border 
                    ...
                    <div className='ml-auto flex items-center'>
                        <div className='flex items-center group px-3 bg-white border border-zinc-900 cursor-pointer h-[45px] rounded 
                            ...
                            <div className='flex justify-between w-full items-center'>
                                <span>Task Type: {type.replace("Tasks", "")}</span>
                            ...
                            <div className='w-[240px] hidden group-hover:block absolute top-[104%] right-0 shadow-2xl rounded-md bg-white z-10'>
                                <div className='w-full flex flex-col max-h-[400px] overflow-y-auto font-medium'>
                                    ...
                                    ${type == "Upcoming Tasks" && "bg-gray-50"}`} onClick={() => TriggerStage("Upcoming Tasks")}>Upcoming Tasks</div>
                                    <div className={`w-full py-4 px-4 border-b border-ray-200 cursor-pointer hover:bg-gray-50 
                                    ${type == "Overdue Tasks" && "bg-gray-50"}`} onClick={() => TriggerStage("Overdue Tasks")}>Overdue Tasks</div>
                                    <div className={`w-full py-4 px-4 border-b border-ray-200 cursor-pointer hover:bg-gray-50 
                                    ${type == "Completed Tasks" && "bg-gray-50"}`} onClick={() => TriggerStage("Completed Tasks")}>Completed Tasks</div>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="w-full mt-3 border border-gray-200 rounded-md overflow-hidden shadow-xl">
                    ...
                    <div className='w-full divide-y divide-gray-200'>
                        {/* Loader */}
                        {!tasks_fetched && <div className='col-span-full h-[250px] bg-white flex items-center justify-center'>
                            ...
                            tasks_fetched && (
                                (tasks.length && tasks.length > 0)
                                    ? (tasks.map((task, index) => {
                                        return (<TaskListCard key={`${task.task_id}-${index}`} prop={task} setCanLoad={setCanLoad}
                ...
                <div className='w-full h-[90px]'>
                    {total_page > 0 ? <Pagination totalPage={total_page} curr_page={initialPage} url_path={`/admin/tasks?type=${type}&`} /> : null}
                </div>
            </div>
            <ToastContainer />
        </div>
