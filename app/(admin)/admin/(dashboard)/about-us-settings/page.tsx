...
const AboutUs = () => {
    ...
    useEffect(() => {
        const fetch_info = async () => {
            ...
            if (comp_info.success && comp_info.data) {
                if (comp_info.data.about_us) {
                    setAboutUs(comp_info.data.about_us)
                }
    ...
    const handleSubmit = async () => {
        ...
        if (!about_us) {
            toast.dismiss();
            toast.error("About us can't be empty", {
                position: "top-center",
                theme: "colored"
            })
            return false;
    ...
    return (
        <div className='w-full'>
            <PageTitle text="About Us Settings" show_back={true} />
            <div className='container m-auto max-w-[99%] lg:max-w-[900px]'>
                <div className='w-full mt-6 border border-gray-200 bg-white p-3 sm:p-7 flex flex-col'>
                    <Ck_Editor_Component data={about_us} onDataChange={handleDataChange} height="400px" />
                    <div className=' mt-6 w-full'>
                        <button className='test_btn bg-gray-800 py-3 px-4 text-white float-right hover:bg-gray-700 hover:drop-shadow-md'
                            onClick={handleSubmit}>Update About Us</button>
                    </div>
                </div>
            </div>
