...
import { APIResponseProps, BlogCategories } from '@/components/types';
...
const EditBlogPost = () => {
    ...
    const [old_categories, setOldCategories] = useState([]);
    ...
    useEffect(() => {
        ...
        const fetchDraftInfo = async () => {
            try {
                ...
                const postPromise: Promise<APIResponseProps> = helpers.GetDraftBlogPostInfo(draft_id);
                ...
                const checked_cats = data_info.categories;
                const blog_menus = data_info.show_on_menus;

                setFormData({
                    post_title: data_info.post_title,
                    slug: data_info.slug,
                    excerpt: data_info.excerpt,
                    post_body: data_info.post_body,
                });

                setCheckedCats([...checked_cats]);
                setCheckedMenus({ ...blog_menus });
                //setSelectedImage(data_info.header_image);
                setHeaderIMage(data_info.header_image);
                setPubHeaderImage(data_info.published_header_image);
                setOldCategories(data_info.categories);
                setDateAdded(data_info.date_added);

                if (checked_cats.length) {
                    checked_cats.forEach((cat: string) => {
                        const this_cat = document.getElementById(`cat_name_${cat}`) as HTMLInputElement;
                        if (this_cat) {
                            this_cat.checked = true;
                        }
                    });
                }

                if (blog_menus.buyer_menu == "Yes") {
                    const this_menu = document.getElementById(`buyer_menu`) as HTMLInputElement;
                    if (this_menu) {
                        this_menu.checked = true;
                    }
                }

                if (blog_menus.seller_menu == "Yes") {
                    const this_menu = document.getElementById(`seller_menu`) as HTMLInputElement;
                    if (this_menu) {
                        this_menu.checked = true;
                    }
                }
    ...
    const handleSubmit = async (type: string) => {
        ...
        try {
            ...
            form_data.append("old_categories", JSON.stringify(old_categories));
            form_data.append("date_added", date_added);

            const { data } = await axios.patch("/api/(blogs)/manage-blog-post", form_data);
            ...
            if (data.success) {
                ...
                if (type == "Publish") {
                    setPubHeaderImage(data.data.header_file_name);
                }
    ...
    if (!post_found) {
        throw new Error("Invalid post info provided");
    }
    ...
    return (
        <div className='w-full'>
            ...
            <div className='container m-auto max-w-[600px] lg:max-w-[1100px]'>
                <PageTitle text="Edit Blog Post" show_back={true} />
